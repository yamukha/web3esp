...
void do_keccak256(uint8_t *hashed, const uint8_t *data_in, size_t data_len)
{
    void *pub_hash;
    sha3_context c;
    sha3_Init256(&c);
    ...
    sha3_Update(&c, data_in, data_len);
    pub_hash = (void *)sha3_Finalize(&c);

    memcpy(hashed, pub_hash, 32);
    return;
}